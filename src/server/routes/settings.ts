...
export async function settingsRoutes(app: FastifyInstance) {
  ...
  const findLocalPostgresBinary = async (
    ...
    if (process.platform === "win32") {
      try {
        ...
        // PowerShell may be unavailable or PostgreSQL may not be running locally.
      }
    }
    ...
    try {
      ...
      // Continue to the Docker runner when the local client is unavailable.
    }
    return null;
  };
  ...
  const runDatabaseCommand = async (
    ...
    // Prefer the PostgreSQL client directly in every runtime. Docker is only
    // an optional fallback for environments where the client is unavailable.
    if (await runLocal()) return;
    if (await runDocker()) return;

    throw new Error(
      process.platform === "win32"
        ? "No active local PostgreSQL instance or usable Docker runner was found."
        : "Neither Docker PostgreSQL runner nor local PostgreSQL client tools are available.",
    );
  };

  app.get(
    "/database-backup",
    { preHandler: databaseAccess },
    async (_req, reply) => {
      ...
      try {
        await runDatabaseCommand("pg_dump", ["--format=custom"], {
          outputPath: filePath,
        });
    ...
    async (req, reply) => {
      ...
      try {
        ...
        const message =
          error instanceof Error
            ? error.message
            : "Panel domain provisioning failed";
        const statusCode =
